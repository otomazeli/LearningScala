...
object ChapterFour {
  ...
  case object Absent extends Maybe[Nothing] {
    def get =
      throw new UnsupportedOperationException("get not defined on Absent")
  }
  ...
  def sequence[A](as: List[Maybe[A]]): Maybe[List[A]] =
    as.foldRight(Just(List.empty[A]): Maybe[List[A]]) {
  ...
  def sequence3[A, B](as: List[Either2[A, B]]): Either2[A, List[B]] = {
    @annotation.tailrec
    def iter(as: List[Either2[A, B]], acc: List[B]): Either2[A, List[B]] =
      as match {
  ...
  sealed trait Validation[+E, +A] {
    ...
    def orElse[EE >: E, AA >: A](other: Validation[EE, AA]): Validation[EE, AA] = {
      if (isSuccess) this
        ...
        (this, other) match {
          case (ErrorTrail(errors), ErrorTrail(moreErrors)) =>
            ErrorTrail(errors ++ moreErrors)
